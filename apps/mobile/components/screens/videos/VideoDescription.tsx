  ...
  return (
    <View style={tw("rounded-3xl bg-white border border-indigo-50 p-6 gap-6")}>
      ...
      <View style={tw("flex-row flex-wrap items-center justify-between gap-4")}>
        ...
        >
          {isFollowLoading ? (
            <ActivityIndicator
              size="small"
              color={isFollowing ? "#4338CA" : "#FFFFFF"}
            />
          ) : isFollowing ? (
      ...
      <View style={tw("flex-row flex-wrap items-center gap-3")}>
        ...
        <Pressable
          onPress={() => onBookmark(video.id, video.isSaved, video)}
          style={tw(
            `flex-row items-center gap-2 px-4 py-2 rounded-full ${video.isSaved ? "bg-indigo-50" : "bg-gray-100"
            }`,
          )}
        >
          <Bookmark size={18} color={video.isSaved ? "#4F46E5" : "#6B7280"} />
          <Text
            style={tw(
              `text-sm font-kh-medium ${video.isSaved ? "text-indigo-600" : "text-gray-700"
              }`,
            )}
          >
            រក្សាទុក
        ...
        <Pressable
          onPress={handleShare}
          style={tw(
            "flex-row items-center gap-2 px-4 py-2 rounded-full bg-gray-100",
          )}
        >
