...
export default function CommentComponent({
  ...
  const [likeCount, setLikeCount] = useState(
    typeof (comment as any).likeCount === "number"
      ? (comment as any).likeCount
      : 0,
  );
  ...
  const fetchReplies = async () => {
    ...
    try {
      ...
      if (commentType === "video") {
        const response = await feedVideoReplyService.getVideoReplies(
          comment.id,
        );
        fetchedReplies = response.data;
      } else {
        const response = await feedForumReplyService.getForumReplies(
          comment.id,
        );
        fetchedReplies = response.data;
      }
  ...
  const handleCommentLike = async (event: GestureResponderEvent) => {
    ...
    try {
      if (commentType === "forum") {
        await meForumCommentService.toggleForumCommentLike(
          comment.id,
          wasLiked,
        );
      } else {
        await meVideoCommentService.toggleVideoCommentLike(
          comment.id,
          wasLiked,
        );
      }
        ...
        {!isReadOnly && isReplying && (
          <View style={tw("mt-3 gap-2")}>
            <View style={tw("relative flex-row items-center gap-2")}>
              <Text
                style={tw(
                  "absolute left-4 top-3 text-xs text-gray-500 text-center font-kh-bold z-10 bg-indigo-50 rounded-full p-1 ",
                )}
              >
              ...
              <TextInput
                ...
                style={[
                  tw(
                    "flex-1 py-3 pr-4 text-sm border border-gray-200 rounded-full bg-white font-kh-medium",
                  ),
      ...
      {isShowingReplies && (
        <View style={tw("mt-3 gap-2")}>
          {repliesError ? (
            ...
            <Text style={tw("text-xs text-gray-400 ml-8")}>
              មិនមានការឆ្លើយតប
            </Text>
          )}
        </View>
