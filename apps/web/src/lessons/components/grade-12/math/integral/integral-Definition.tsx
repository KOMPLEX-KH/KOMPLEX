...
`}
          ...
          <p className="text-sm text-slate-700">
            បើ <InlineMath math="f" /> ជាប់លាប់លើ <InlineMath math="[a,b]" />{" "}
            នោះវា integrable (អាចយកអាំងតេក្រាលបាន) លើចន្លោះនោះ។
          </p>
        </div>
        ...
        <div className="rounded-xl border-l-4 border-emerald-500 bg-emerald-50/60 p-4 shadow-sm space-y-2">
          <p className="font-semibold text-slate-800">
            ទ្រឹស្តីមូលដ្ឋាននៃកាលគុលូស (FTC)
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-lg bg-white/70 border border-emerald-200 p-3">
              <p className="text-sm font-medium text-slate-700 mb-1">
                ផ្នែក I (អនុគមន៍សមាត្រ)
              </p>
              <p className="text-sm mb-1">
                កំណត់ <InlineMath math={String.raw`G(x)=\int_a^x f(t)\,dt`} />។
                បើ <InlineMath math="f" /> ជាប់លាប់ នៅពេលនោះ{" "}
              </p>
              <MathLine math={String.raw`G'(x)=f(x)`} />
            </div>
            <div className="rounded-lg bg-white/70 border border-emerald-200 p-3">
              <p className="text-sm font-medium text-slate-700 mb-1">
                ផ្នែក II (បញ្ចូលខ្ទង់)
              </p>
              <p className="text-sm mb-1">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="rounded-lg border-l-4 border-indigo-500 bg-indigo-50/70 p-3 shadow-sm">
            ...
            <MathLine
              math={String.raw`\int_a^b (c\,f+g)\,dx=c\!\int_a^b f\,dx+\int_a^b g\,dx`}
            />
          </div>
          ...
          <div className="rounded-lg border-l-4 border-amber-500 bg-amber-50/70 p-3 shadow-sm">
            ...
            <MathLine
              math={String.raw`\int_a^b f\,dx=\int_a^c f\,dx+\int_c^b f\,dx`}
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="rounded-lg border-l-4 border-sky-500 bg-sky-50/70 p-3 shadow-sm">
            <p className="font-semibold text-sky-800 mb-1">
              ប្រៀបធៀប និង​ដែនកំណត់
            </p>
            <MathLine
              math={String.raw`m\le f(x)\le M\ \Rightarrow\ m(b-a)\le \int_a^b f(x)\,dx\le M(b-a)`}
            />
            <MathLine
              math={String.raw`|{\textstyle\int_a^b} f\,dx|\ \le\ \int_a^b |f|\,dx`}
            />
          </div>
          <div className="rounded-lg border-l-4 border-fuchsia-500 bg-fuchsia-50/70 p-3 shadow-sm">
            ...
            <MathLine
              math={String.raw`\int_{-a}^{a}\text{even}(x)\,dx=2\int_0^{a}\text{even}(x)\,dx`}
            />
          </div>
        </div>
        ...
        <div className="rounded-lg border-l-4 border-violet-500 bg-violet-50/70 p-3 shadow-sm">
          <p className="font-semibold text-violet-800 mb-1">
            តម្លៃមធ្យមនៃអនុគមន៍លើ [a,b]
          </p>
          <MathLine
            math={String.raw`f_{\text{avg}}=\frac{1}{b-a}\int_a^b f(x)\,dx`}
          />
        </div>
      </div>
    ),
    ...
    steps: [
      ...
      },
      {
        ...
        content: (
          <div>
            ...
            <MathLine
              math={String.raw`\int_{0}^{\pi/2}\sin(2x)\,dx=\frac{1}{2}\int_{0}^{\pi}\sin u\,du`}
            />
            <MathLine
              math={String.raw`=\frac{1}{2}\big[-\cos u\big]_{0}^{\pi}=\frac{1}{2}\,( -(-1)-(-1) )=1`}
            />
          </div>
        ),
      },
      {
        ...
        content: (
          <div>
            <p>
              រវាង <InlineMath math="y=x" /> និង <InlineMath math="y=x^2" /> លើ
              [0,1]: តំបន់ = ∫(លើ−ក្រោម)។
            </p>
            <MathLine
              math={String.raw`\int_{0}^{1}\big(x-x^2\big)\,dx=\Big[\tfrac{x^2}{2}-\tfrac{x^3}{3}\Big]_0^1=\tfrac{1}{2}-\tfrac{1}{3}=\tfrac{1}{6}`}
            />
          </div>
        ),
      },
    ...
    content: (
      <div className="space-y-3">
        <div className="rounded-lg border-l-4 border-sky-500 bg-sky-50/70 p-3 shadow-sm">
          ...
          <p>
            ១) រកអន្តរអនុគមន៍ <InlineMath math="F" /> ដែល{" "}
            <InlineMath math="F'=f" /> → បញ្ចូលខ្ទង់{" "}
            <InlineMath math="F(b)-F(a)" />។
          </p>
          <p>
            ២) សម្រាប់ integrand ដែលសមស្របនឹង u-substitution ឬ parts ចូលរួម
            សូមបម្លែងជាមុនសិន។
          </p>
        </div>
        <div className="rounded-lg border-l-4 border-emerald-500 bg-emerald-50/70 p-3 shadow-sm">
          <p className="font-semibold text-slate-800 mb-1">
            ទម្រង់ពិសេស (កែព្រំដែន)
          </p>
          <MathLine
            math={String.raw`u=g(x),\ du=g'(x)\,dx\ \Rightarrow\ \int_{x=a}^{x=b} f(g(x))\,g'(x)\,dx=\int_{u=g(a)}^{u=g(b)} f(u)\,du`}
          />
          <MathLine
            math={String.raw`\int_a^b u\,dv=\Big[u\,v\Big]_a^b-\int_a^b v\,du`}
          />
        </div>
      </div>
    ),
  ...
  warning: {
    content: (
      <div className="space-y-1.5">
        <p>
          • កុំបន្ថែម +C សម្រាប់អាំងតេក្រាល<strong>កំណត់</strong> (មានព្រំដែន)។
        </p>
        <p>
          • ប្ដូរទិសខ្ទង់នាំឲ្យប្ដូរសញ្ញា៖{" "}
          <InlineMath math={String.raw`\int_a^b f\,dx=-\!\int_b^a f\,dx`} />។
        </p>
        <p>
          • ពេល u-substitution សម្រាប់អាំងតេក្រាលកំណត់ ត្រូវកែ <em>ព្រំដែន</em>{" "}
          ទៅតាម u ជានិច្ច។
        </p>
