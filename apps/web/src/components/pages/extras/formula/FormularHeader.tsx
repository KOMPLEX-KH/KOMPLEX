...
export default function FormularHeader({
  ...
  return (
    <div className="mb-6 dark:bg-zinc-900" ref={panelRef}>

      {/* Subject pills */}
      <div className="flex gap-2 mb-4 overflow-x-auto dark:bg-zinc-900">
        {subjects.map(s => {
          const Icon = s.icon;
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-full flex gap-2 items-center text-sm font-medium transition
                ${active ? "bg-indigo-600 text-white dark:text-zinc-900" : "bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 hover:bg-indigo-50"}`}
            >
      ...
      <div className="flex gap-3 relative">
        ...
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 border border-gray-400 dark:border-zinc-800 rounded-full focus:outline-none"
          />
        </div>

        <button
          onClick={() => setFilterOpen(!filterOpen)}
          className="bg-indigo-600 text-white dark:text-zinc-900 p-4 rounded-full"
        >
        ...
        {filterOpen && (
          <div
            ref={filterPanelRef}
            className="absolute bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-800 z-[999] p-4 animate-slide-down"
            style={{
              top: "calc(100% + 10px)",
              right: "0",
              width: "280px",
          ...
          >
            {/* Subject Dropdown */}
            <div className="mb-4">
              <label className="flex text-sm font-semibold text-gray-700 dark:text-zinc-400 mb-2  items-center gap-2">
                មុខវិជ្ជា
              </label>
              <div className="relative" ref={subjectDropdownRef}>
                <button
                  onClick={() => setOpenSubjectDropdown(!openSubjectDropdown)}
                  className="w-full relative appearance-none bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 py-2.5 px-4 pr-10 rounded-xl cursor-pointer transition-all duration-200 text-gray-700 dark:text-zinc-400 font-medium outline-none flex items-center justify-between"
                >
                ...
                {openSubjectDropdown && (
                  <div className="absolute top-full mt-1 w-full bg-white dark:bg-zinc-900 rounded-xl shadow-xl border border-gray-200 dark:border-zinc-800 z-50 overflow-hidden">
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 transition-all duration-150 text-sm font-medium ${isSelected
                              ...
                              : 'hover:bg-gray-50 border-l-4 border-transparent text-gray-700 dark:text-zinc-400'
                              }`}
                          >
            ...
            {/* Category Dropdown */}
            <div className="mb-4">
              <label className="block text-sm font-semibold text-gray-700 dark:text-zinc-400 mb-2">មេរៀន</label>
            ...
            <button
              ...
              className="w-full flex hover:bg-indigo-700 dark:hover:bg-indigo-600 items-center justify-center gap-3 bg-indigo-600 dark:bg-indigo-900 text-white dark:text-zinc-900 py-3 rounded-xl font-semibold transition-all duration-300"
            >
              <Filter className="w-5 h-5 text-white dark:text-zinc-900" />
              ស្វែងរក
            </button>
          </div>
        )}
