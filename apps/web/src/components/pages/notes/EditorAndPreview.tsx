...
import { Calculator, Code, Eye, AlertCircle } from 'lucide-react'
...
interface EditorAndPreviewProps {
    initialCode?: string;
    onContentChange?: (content: TopicContent_V3[]) => void;
}

export default function EditorAndPreview({
    initialCode = START_CODE,
    onContentChange
}: EditorAndPreviewProps) {
    ...
    const evaluateCode = useCallback((codeString: string): TopicContent_V3[] | null => {
        ...
        try {
            ...
            const result = userFunction(
                ...
                InlineMath,
                BlockMath,
                // this somehow makes proxy works
                Calculator,
                DefinitionBox,
                TipBox,
                ExampleBox,
    ...
    return (
        <div className="w-full  mx-auto p-6 pt-15 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {isEvaluating && (
                                <div className="flex items-center gap-2 ml-auto">
                                    ...
                                    <span className="text-white text-sm">កំពុងដំណើរការ...</span>
                                </div>
                            )}
                            {error && !isEvaluating && (
                                <div className="flex items-center gap-2 ml-auto">
                                    <AlertCircle className="w-4 h-4 text-red-200" />
                                    <span className="text-red-200 text-sm">មានកំហុស</span>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-3xl shadow-lg border border-gray-200 overflow-hidden">
                    {/* Preview Header */}
                    <div className={`${error ? 'bg-red-600' : 'bg-green-600'} px-6 py-4`}>
                        <div className="flex items-center gap-3">
