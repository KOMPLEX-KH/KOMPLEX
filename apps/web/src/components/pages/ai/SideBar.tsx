...
const SideBar: React.FC<SideBarProps> = ({ onCollapsedChange }) => {
    ...
    return (
        <>
            ...
            {/* Edit modal */}
            {editingId !== null && (
                <div className="fixed inset-0 z-[1300] bg-black/30 backdrop-blur-sm flex items-center justify-center px-4 pointer-events-auto">
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-sm p-4 space-y-3">
                        <div className="text-sm font-semibold text-gray-800">កែប្រែឈ្មោះសន្ទនា</div>
                        <input
                            ref={editModalInputRef}
                            type="text"
                            value={editValue}
                            onChange={(e) => setEditValue(e.target.value)}
                            className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        <div className="flex justify-end gap-2">
                            <button
                                ...
                                className="inline-flex items-center justify-center w-9 h-9 rounded-full border border-gray-200 text-gray-600 hover:bg-gray-100 transition-colors"
                                aria-label="បោះបង់"
                            >
                                <X className="w-4 h-4" />
                            </button>
