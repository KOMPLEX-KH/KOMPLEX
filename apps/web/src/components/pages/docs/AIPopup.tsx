...
export default function AIPopup({ isOpen, onClose }: AIPopupProps) {
  ...
  const [pendingResponseType, setPendingResponseType] =
    useState<AIResponseType | null>(null);
  ...
  const parsedTopicId = Array.isArray(topicParam)
    ? Number(topicParam[0])
  ...
  const isTopicScoped =
    typeof topicId === "number" && !Number.isNaN(parsedTopicId);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const subject = grade?.subjects?.find(
        (s: Subject) => s.id === Number(params?.subject),
      );
      const lesson = subject?.lessons?.find(
        (l: Lesson) => l.id === Number(params?.lesson),
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const history = await meAiService.getAiGeneralHistoryBasedOnTopic(
          topicId,
          1,
          20,
        );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response: ApiWrapper<AIResponse> = await meAiService.callAiTopic(
        payload,
        topicId,
          ...
          <>
            ...
            >
              ...
              >
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 rounded-t-3xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-1">
                      ...
                      <span className="text-indigo-500 font-bold"> AI 1.0</span>{" "}
                      - {topicTitle}
                    </div>
                  </div>
                ...
                >
                  <div
                    className={`space-y-4 ${isPopupMaximized ? "max-w-4xl mx-auto" : ""}`}
                  >
                    {isHistoryLoading ? (
                      <ChatSkeleton />
                      ...
                      <>
                        ...
                        {isLoading && (
                          <ResponseLoadingState
                            responseType={
                              pendingResponseType ?? selectedResponseType.id
                            }
                          />
                        )}
                        {isStreaming && (
                ...
                {/* Input */}
                <div
                  className={`lg:px-0 px-4 ${isPopupMaximized ? "max-w-4xl mx-auto w-full" : ""}`}
                >
                  {activeRating ? (
                    <AiRating
